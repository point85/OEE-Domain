...
public class TestClassification extends BaseTest {
	...
	public void testClassifications() throws Exception {
		...
		// electromotive force
		assertTrue(kg.multiply(m.power(2)).divide(amp).divide(s.power(3)).classify().getUnitType()
				.equals(UnitType.ELECTROMOTIVE_FORCE));

		// electric resistance
		assertTrue(kg.multiply(m.power(-3)).multiply(amp.power(2)).multiply(s.power(4)).classify().getUnitType()
				.equals(UnitType.ELECTRIC_RESISTANCE));

		// electric capacitance
		assertTrue(s.power(-3).multiply(amp.power(-2)).multiply(m.power(2)).divide(kg).classify().getUnitType()
				.equals(UnitType.ELECTRIC_CAPACITANCE));

		// electric permittivity
		assertTrue(s.power(4).multiply(amp.power(2)).multiply(m.power(-3)).divide(kg).classify().getUnitType()
				.equals(UnitType.ELECTRIC_PERMITTIVITY));

		// electric field strength
		assertTrue(amp.divide(m).classify().getUnitType().equals(UnitType.ELECTRIC_FIELD_STRENGTH));

		// magnetic flux
		assertTrue(kg.divide(amp).divide(s.power(2)).multiply(m.power(2)).classify().getUnitType()
				.equals(UnitType.MAGNETIC_FLUX));

		// magnetic flux density
		assertTrue(kg.divide(amp).divide(s.power(2)).classify().getUnitType().equals(UnitType.MAGNETIC_FLUX_DENSITY));

		// inductance
		assertTrue(kg.multiply(amp.power(-2)).divide(s.power(2)).multiply(m.power(2)).classify().getUnitType()
				.equals(UnitType.ELECTRIC_INDUCTANCE));

		// conductance
		assertTrue(kg.power(-1).multiply(amp.power(2)).multiply(s.power(3)).multiply(m.power(-2)).classify()
				.getUnitType().equals(UnitType.ELECTRIC_CONDUCTANCE));
		...
		assertTrue(sys.getTypeMap(UnitType.PLANE_ANGLE).isEmpty());
		assertTrue(sys.getUOM(Unit.RADIAN).getBaseUnitsOfMeasure().isEmpty());

		// solid angle
		assertTrue(sys.getTypeMap(UnitType.SOLID_ANGLE).isEmpty());
