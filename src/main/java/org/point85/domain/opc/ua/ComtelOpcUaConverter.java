...
public class ComtelOpcUaConverter {
	...
	public static Object toWritableDataTypeObject(NodeId node, String value) throws Exception {
		...
		case 13:
			return new DateTime(
					java.util.Date.from(ZonedDateTime.from(DateTimeFormatter.ISO_DATE_TIME.parse(value)).toInstant()));
		case 14:
	...
	public static String toString(ExtensionObject ext) {
		if (ext.getEncodingTypeId() != null && ext.getEncodingTypeId().getIdentifier() != null) {
			if (Identifiers.Range_Encoding_DefaultBinary.getIdentifier()
					.equals(ext.getEncodingTypeId().getIdentifier())) {
				return toRangeString((ByteString) ext.getEncoded());
			}
			if (Identifiers.EUInformation_Encoding_DefaultBinary.getIdentifier()
					.equals(ext.getEncodingTypeId().getIdentifier())) {
